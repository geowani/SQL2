...
using namespace std;
	class clempresa{
		...
		char* conectar(conectar,"localhost","usr_empresa", "Empres@123", "db_empresa,marca", 3306, NULL,0 ){
			
				if(!(obj=mysql_init(0)))
					{
						return "Error Create Objeto";
					}
					if(!mysql_real_connect("localhost","usr_empresa", "Empres@123", "db_empresa.marca", 3306, NULL,0 ) )
					{
	...
	class clempresa{
		public:
			...
			void insertar(struct stempresa ){
				try{
				conexion.conectar("localhost","usr_empresa", "Empres@123", "db_empresa.marca");
				char *consulta;
				char sentencia[]="INSERT INTO  db_empresa(ID,NOMBRE,PATERNO) VALUES (\'%s\',\'%s\',\'%s\')";
			...
			void mostrar(){
				   ...
				   if(!mysql_real_connect(objDatos,"localhost", "usr_empresa", "Empres@123", "db_empresa.marca", 3306, NULL, 0)) {
